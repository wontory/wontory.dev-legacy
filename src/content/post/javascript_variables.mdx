...
이렇게 호이스팅은 프로그래밍에 안좋은 영향을 준다. 우리는 `var` 대신 **TDZ**를 갖춘 `let`과 `const`를 사용해 호이스팅으로부터 조금은 자유로워질 수 있다.

&nbsp;

> 참고: [TDZ을 모른 채 자바스크립트 변수를 사용하지 말라](https://ui.toast.com/weekly-pick/ko_20191014)
